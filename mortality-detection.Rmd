...
ground_test <- read_csv("ground_test_2_rformat.csv", 
  ...
  group_by(ID) %>%
  mutate(ranks = row_number()) %>%
  mutate(known = ifelse(ranks == max(ranks), 2, NA)) %>% 
  ungroup() %>% 
  dplyr::select(-ranks)
...
```{r}
stateNames <- c("live mig", "dead") #"live stop"
dist <- list(step="gamma", angle="wrpcauchy", mean_dist_5 = "norm")

fixPar <- list(beta=matrix(c(NA, -1000), nrow = 1, byrow = TRUE))
# NA,	NA,	NA,	NA,	-100,	-100
Par0_m1 <- list(step=c(5, 0.0137,
                       100, 0.0233,
                       0.1, 0.1),
                angle = c(0.001, 0.001,
                          0.99, 0.99),
                mean_dist_5 = c(mean(amwo_hmm$mean_dist_5), 5.72,
                                sd(amwo_hmm$mean_dist_5), 14.7))
ks <- amwo_hmm$known
```
